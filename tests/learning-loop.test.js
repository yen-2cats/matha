...
test('GPT-5.5 整卷批改只保存分數、錯題與安全紅筆標記，隔天才開答案', () => {
  const { run } = loadApp();
  const result = plain(run(`(() => {
    save = () => {};
    ...
    return { status:row.status, due:row.due, today:today(), score:row.score, wrong:row.wrongNos, ext:S.extMocks[0], labels:grade.questions.flatMap((q) => q.marks.map((m) => m.label)), model:grade.model };
  })()`));
...
test('整卷結果直接疊加 AI 紅筆畫布，不再出現答案卡或人工分數欄位', () => {
  const { context, run } = loadApp();
  context.__app = { innerHTML: '' };
  context.document.querySelector = (selector) => selector === '#app' ? context.__app : null;
  const html = run(`(() => {
  ...
  assert.match(html, /GPT‑5\.5 紅筆批改/);
