...
function qProcReview(ok) {
  const sess = qsess;
  const q = sess.q;
  const calcB64 = inkCapture(q.id, 's') || inkCapture(q.id, 'q'); // 只寫在題目區也分析
...
function mockJudgePanel(list) {
  ...
  const items = list.map((q, i) => {
    const img = inkCapture(q.id, 's', true) || inkCapture(q.id, 'q', true);
...
async function mockAIJudge() {
  ...
  for (let i = 0; i < m.toJudge.length; i++) {
    ...
    try {
      const v = await aiGradeCall(q, q.ans.join(' 或 '), inkCapture(q.id, 's') || inkCapture(q.id, 'q'));
      if (mock !== m || sessionMode !== 'judging') return;
      m.aiv[q.id] = v;
      const box = $('#jai-' + i);
