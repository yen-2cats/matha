...
目前救援版為 `0716c`。全站已重做為暖白、石墨字與低彩度灰褐／橄欖色的簡潔介面；導覽與狀態圖示使用專案內建的 SVG，不以 emoji 當正式 UI。桌機使用頂部導覽，手機改為底部可橫向滑動的功能列；作答時則把空間完整留給題目與計算紙。
...
本次搶救的 findings、驗證範圍與未驗證邊界見 `RESCUE_AUDIT_2026-07-16.md`；更完整的系統沿革與設計決策見 `WEB_APP_DESIGN.md`。其中舊行號與舊部署拓撲可能已過時，實作以目前程式與測試為準。
